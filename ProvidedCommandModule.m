...
@implementation ProvidedCommandModule
...
- (instancetype)init
{
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reconfigureView) name:@"easycommand/ReloadSmileys" object:nil];
...
{
	NSLog(@"Smiley: settingsIdentifier %@", self.settingsIdentifier);
	NSString *str = [NSString stringWithFormat:@"%@.run", self.settingsIdentifier];
